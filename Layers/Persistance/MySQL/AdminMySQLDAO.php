...
include_once (ROOT .'/Layers/Persistance/dao/AdminDAO.php');

/**
 * Description of AdminMySQLDAO
 *
 * @author jorge
 */
class AdminMySQLDAO extends AdminDAO {
    ...
    public function clean() {          
        $query = "DELETE FROM {$GLOBALS['DB_NOMBRE']}.Professor WHERE idUser IN ("
        . "SELECT idUser FROM {$GLOBALS['DB_NOMBRE']}.Admin)";
    ...
    public function delete($idUser) {
       $query = "DELETE u FROM {$GLOBALS['DB_NOMBRE']}.User u 
                INNER JOIN {$GLOBALS['DB_NOMBRE']}.Admin  a 
    ...
    private function convertMySQLObjectCurso($row){
        ...
        return new Admin($idUser, $password, $name, $lastName1, $lastName2, $dni, $profile_avatar, $mail,$phone,$idRol);
        
    }
    
    /**
     * Obtiene un objeto Admin desde la Base de Datos
     * @param int $idUser
     * @return Admin
     */
    ...
    public function getAll() {
        $query = "SELECT * FROM {$GLOBALS['DB_NOMBRE']}.User u JOIN {$GLOBALS['DB_NOMBRE']}.Admin a ON a.idUser = u.idUser";        
        return $this->getAllByQuery($query);
        
    }

    /**
     * Obtiene todos los objetos Admin de la base de datos ordenados en base
    ...
    public function getAllOrderedBy($param, $order = 0) {
        $query = "SELECT * FROM {$GLOBALS['DB_NOMBRE']}.User u JOIN {$GLOBALS['DB_NOMBRE']}.Admin a ON a.idUser = u.idUser ORDER BY $param ";
        if($order == 0){
            $query .="ASC";
        }
        else{
    ...
    public function insert($object, $isUserInserted = false) {
        if(!$isUserInserted){
            $query = "INSERT INTO {$GLOBALS['DB_NOMBRE']}.User (name,lastName1,lastName2,dni,mail,password,profile_avatar,phone) "
        ...
        if($idUsuario != FALSE){
            ...
            $queryS = "INSERT INTO {$GLOBALS['DB_NOMBRE']}.Admin (idUser) VALUES ({$idUsuario})";
